...
1. Zastanów siê nad mo¿liwoœci¹ rozszerzenia typu `Ulamek` na typ `UlamekPro` przez dziedziczenie, wprowadzaj¹c dodatkow¹ funkcjonalnoœæ: 

    mieszana reprezentacja tekstowa u³amka (znak, czêœæ ca³kowita, spacja, czêœæ u³amkowa zapisana w formie u³amka w³aœciwego)
    
    ````[znak][czêœæ ca³kowita] <<licznik>>/<<mianownik>>````

    np. `-2 3/4` dla u³amka niew³aœciwego, ale dla u³amka w³aœciwego `3/4`.
